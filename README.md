...
* Creating a hierarchical multi-language RouteTree using an expressive syntax (mostly mimicing Laravel's own). That hierarchy is then the basis for:
    * Unique language-agnostic route IDs (e.g. `company.team.contact`) to be used for various purposes (e.g. any kind of navigation, language-switches, breadcrumb-menus, hierarchical sitemaps, etc.) as well as any other custom information). Syntax examples:
        * Language-neutral link to specific nodes of the RouteTree: `route_node_url('company.team.contact')`.
        * Link to current page in other language: `route_node_url()->locale('de')`.
        * Accessing hierarchical parents/siblings/children: e.g. `route_tree()->getCurrentNode->getChildNodes()`
        * Accessing any other kind of custom information: `route_node_id
    * Automatic generation of routes for all configured languages with the locale as the first path-segment (e.g. `en/company/team/contact`).
    * Automatic generation of expressive route names (e.g. `en.company.team.contact.get`).
    * Automatic translation (e.g. of path-segments, page-title or any other custom information) via structured language-files, that mirror the hierarchical RouteTree structure.
...
* Automatic translation:
    * Using . (You can also utilize this structure for page-content using the included `trans_by_route()` helper.)
...
* Language-neutral links (e.g. `route_node_url('company.team.contact')` will create a link in the currently set locale).
* Automatic locale setting:
    * From a localized Route generated by RouteTree. (e.g. `en.company.news.get`).
    * From a (previously saved) session value.
    * From a `HTTP_ACCEPT_LANGUAGE` header sent by the client.
* XML-Sitemap generation:
    * Automatic exclusion of `auth` routes and redirects.
    * Manual exclusion of routes.
    * Resolving of all possible route keys for parameter/resource routes.
    * Setting of optional tags (lastmod, changefreq, priority) via fluent setters, or Eloquent Models (for parameter/resource routes).
* Cacheable (in combination with Laravel's route caching).
* REST-API to retrieve list of routes.
* RouteTree can be used to easily create:
